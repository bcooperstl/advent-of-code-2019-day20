...
int main (int argc, char * argv[])
{
        ...
        for (int col=1; col<file_col-1; col++)
        {
            if (buffer[row][col]>='A' && buffer[row][col]<='Z')
            {
                ...
                if (should_add)
                {
                    ...
                    if (row==1 || row==file_row-2 || col==1 || col==file_col-2)
                        u.myMap.layout[dest_row][dest_col].portal_direction=OUTSIDE;
                    else
                        u.myMap.layout[dest_row][dest_col].portal_direction=INSIDE;
                }
            }
        }
    }
    ...
    display_map(&u.myMap, WITH_PORTAL_DIRECTIONS);
    path best_path;
    depth_find_best_path(&u, &best_path);
    printf("The best path is: ");
    depth_print_path(&best_path);
